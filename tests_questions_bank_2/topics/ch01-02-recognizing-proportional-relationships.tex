...
\correctAnswer{D}
\explanation{The two given points determine the line uniquely: slope $= \frac{12-6}{4-2} = 3$, so using $(2,6)$ gives $6 = 3(2) + b$, which means $b = 0$. The only possible $y$-intercept is $(0,0)$, so the statement that the line does not pass through the origin is inconsistent with the coordinates in the stem.}
\end{practiceQuestion}
