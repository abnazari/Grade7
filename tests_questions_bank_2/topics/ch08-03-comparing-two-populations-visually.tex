...
\end{questionText}
\correctAnswer{Yes; the difference is $1$ MAD.}
\explanation{Difference: $72 - 65 = 7$. In MADs: $\frac{7}{7} = 1$. A difference of $1$ MAD shows some separation, though there is still considerable overlap. Many guidelines consider $1$ MAD borderline.}
\end{practiceQuestion}
...
\begin{tikzpicture}[scale=0.5]
  ...
  % Club A: 5,6,7,7,8,8,8,9,10 → mean=68/9≈7.56, use mean=8, MAD calculation
  % Actually let me use: 5,6,7,7,8,8,8,9,10 sum=68 mean=68/9 not clean
  % Use: 6,7,7,8,8,8,9,9,10 sum=72 mean=8
  \fill[funBlue] (6,0.5) circle (4pt);
  ...
  % Club B: 8,9,10,10,11,11,11,12,13 sum=95, not 9 values... 95/9 not clean
  % Use: 9,10,10,11,11,11,12,12,13 sum=99 mean=11
  ...
  \node[funRed,above] at (11,2) {\small Club B};
\end{tikzpicture}
\end{center}

Find the mean for each club. Then find the MAD for Club A and express the difference in means as a multiple of the MAD.
\end{questionText}
\correctAnswer{Club A mean $= 8$, Club B mean $= 11$; MAD of Club A $\approx 1.1$; difference $\approx 2.7$ MADs}
